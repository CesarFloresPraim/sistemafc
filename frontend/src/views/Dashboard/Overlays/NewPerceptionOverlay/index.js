...
export default function NewPerceptionOverlay({
  showOverlay,
  perceptionType,
  isEditting = false,
  onSave,
  ...
  const handleSave = () => {
    let newPerceptionType = {
        name: name
    }
    if(isEditting && perceptionType.id) {
        newPerceptionType['id'] = perceptionType.id
    }

    onSave(newPerceptionType, isEditting)
  };

  useEffect(()=> {
    if(isEditting && perceptionType) {
        setName(perceptionType.name)
    }
  }, [perceptionType])
  return (
    <>
        ...
        <div className="w-1/2 bg-white flex flex-col overflow-scroll hide-scrollbar">
          <div className="flex px-16 py-16 items-center">
            <h2 className=" text-2xl md:text-[40px] font-semibold text-left">
              {!isEditting
                ? "NUEVO TIPO DE PERCEPCION"
                : "EDITAR TIPO DE PERCEPCION"}
            </h2>
            <div
              onClick={() => showOverlay(false, false)}
