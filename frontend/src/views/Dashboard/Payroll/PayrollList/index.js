...
export default function PayrollList() {
  ...
  const { payrollList } = useSelector((state) => state.payroll);
  ...
  useEffect(() => {
    dispatch(FetchPayrollList()).then((res) => {
      dispatch(SetPayrollList(res.data));
    });
  }, []);
                    ...
                    <div className="flex gap-4 basis-full">
                      ...
                      <div className="flex flex-col ml-4 justify-center">
                        <div className="text-mineShaft text-base font-semibold">
                          {new Date(
                            item.registerRH.fromDate
                          ).toLocaleDateString("es-ES", dateOptions)}{" "}
                          -{" "}
                          {new Date(item.registerRH.toDate).toLocaleDateString(
                            "es-ES",
                            dateOptions
                          )}
                        </div>
                      ...
                      <div className="flex items-center ml-auto">
                        <button
                          onClick={() => {
                            dispatch(SetSelectedPayroll(item.id))
                            navigate({ pathname: "/detalles-nomina" });
                          }}
                          type="button"
      ...
      <div className="mt-6">
        ...
        >
          <button
            type="button"
            onClick={() => {
              navigate({ pathname: "/nueva-nomina" });
            }}
